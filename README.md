...
핸드폰·아이패드 카메라로 영어를 비추면

- 왼쪽: 실시간 카메라
- 오른쪽 위(2/3): 한글 번역
- 오른쪽 아래(1/3): 영어
- 문장 옆 **단어 / 문법** 버튼: 누르면 그 문장의 뜻과 해석 방법
...
2. 영어가 노란 점선(기본은 화면 중앙) 안에 들어오게 맞춘 뒤 **지금 인식**을 누릅니다.
3. 계속 자동으로 보려면 **1초마다 인식**을 켭니다. 화면이 같으면 결과를 유지하고, 글자가 바뀌면 갈아끼웁니다.
4. 인식 범위는 `중앙만 / 화면 전체 / 하단만 / 상단만`에서 바꿀 수 있습니다.
...
git init -b main
git add .
git commit -m "Add live camera English translator web app"
...
- 장식 폰트, 이펙트, 작은 글자는 가끔 오인식이 납니다. 화면을 가까이 비추거나, 글자가 너무 많으면 영역을 `하단만`으로 줄이면 됩니다.
